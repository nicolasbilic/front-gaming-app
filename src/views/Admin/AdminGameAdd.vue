<template>
  ...
  <section>
    <div>
      ...
      <form @submit.prevent="validateAndAddToAPI" @keydown="handleKeyDown">
        <div class="field-main-container">
          <div class="field-container">
            <label for="title">Titre</label>
            <input type="text" id="title" name="title" placeholder="titre de mon jeu" v-model="game.title"
            ...
            <textarea id="genreDescription" v-model="game.genre.description" required></textarea>
            <label for="genreImages">Genre Images:</label>
            <input type="text" id="genreImages" name="genreImages" placeholder="Images du Genre" v-model="game.genre.images"
              required> -->

            <label for="price" class="price-label">Prix</label>
            <input type="text" id="price" name="price" placeholder="Prix" v-model="formattedPrice"
            ...
            <input type="text" id="discount" name="discount" placeholder="Réduction" v-model="formattedDiscount"
          ...
          <div class="field-container">
            ...
            <input type="date" id="releasedate" name="releasedate" @input="resetSuccessMessage"
              placeholder="Date de sortie" v-model="game.release_date" required>
          </div>
        </div>
        ...
        <div class="field-main-container">
          <div class="field-container">
            ...
            <input type="text" id="features" name="features" placeholder="fonctionnalité" @input="resetSuccessMessage"
              v-model="newFeature">
        ...
        <div class="field-secondary-container">
          ...
          <textarea class="description-textarea" name="description" id="description" cols="30"
            ...
            @input="limitDescription; resetSuccessMessage" required>
          </textarea>
          <span>{{ remainingCharacters }} mots restants.</span>

          <label for="imageUpload">Image (min: 390x390)</label>
          <input type="file" id="imageUpload" name="image" accept="image/jpeg" @input="resetSuccessMessage"
...
const imagePath = ref(''); // ref pour stocker l'URL de l'image téléchargée
const featImgPath = ref(''); // ref pour stocker l'URL de l'image secondaire téléchargée
const errorMessage = ref(''); // ref pour stocker les messages d'erreurs du téléchargement de l'image
...
const addToAPI = () => { // ajoute un jeu
  const newGame: Game = {
    title: sanitizeInput(game.value.title),
    genre: game.value.genre,
    price: game.value.price,
    rating: sanitizeInput(ratingImage.value),
    description: sanitizeInput(game.value.description),
    img: sanitizeInput(game.value.img),
    developer: sanitizeInput(game.value.developer),
    discount: game.value.discount,
    platform: sanitizeInput(game.value.platform),
    features: game.value.features,
    release_date: game.value.release_date,
    images: game.value.images,
  };
  console.log(newGame);
  axios.post(`${apiBaseUrl}/api/add`, newGame, {
    headers: { 'Access-Control-Allow-Origin': 'https://videogame-web-app.vercel.app' }
  })
    .then((response) => {
      successMessage.value = 'Votre jeu a été créé avec succès';
...
const validateForm = () => {
  validatePrice();
  validateDiscount();
  return !priceError.value && !discountError.value;
};

const validateAndAddToAPI = () => { // vérification JS & soumission du formulaire
  if (validateForm()) {
    addToAPI();
  } else {
...
const deleteImage = () => { // fais disparaître l'image affichée
  imagePath.value = '';
  game.value.img = '';
};

const addFeature = () => { // ajout de fonctionnalités
...
const fetchGenreData = async () => { // récupère la description et les images du genre sélectionné
...
const remainingCharacters = computed(() => { // calcule le nombre de caractères restants
  return maxDescriptionLength - game.value.description.length;
});

const ratingImage = computed(() => { // récupère l'image associée au rating
  const index = listRatings.indexOf(game.value.rating); // nous cherchons l'indice de la valeur de notation actuelle 
  return index !== -1 ? imgRatings[index] : ''; // renvoie l'adresse d'image correspondante dans imgRatings à cet indice
});

const handleKeyDown = (event: KeyboardEvent) => { // empêche le form d'être envoyé en appuyant sur la touche enter
  if (event.key === 'Enter') {
